...
<div class="max-w-2xl py-6">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-primary font-heading">Edit Kategori</h1>
            <p class="text-text-muted">Ubah data kategori</p>
        </div>
    </div>
    ...
    <div class="bg-surface rounded-2xl shadow-sm border border-border-subtle p-6">
        ...
        <form action="<?php echo base_url('admin/kategori/edit/'.$kategori->id_kategori); ?>" method="post">
            <div class="mb-5">
                ...
                <input type="text" name="nama_kategori" value="<?php echo $kategori->nama_kategori; ?>" required class="w-full px-4 py-3 rounded-xl border border-border-subtle focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary bg-surface shadow-sm transition">
            </div>
            <div class="mb-5">
                <label class="block text-sm font-semibold text-text-main mb-2 flex items-center gap-2">
                ...
                <textarea name="deskripsi" rows="3" class="w-full px-4 py-3 rounded-xl border border-border-subtle focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary bg-surface shadow-sm transition resize-none"><?php echo $kategori->deskripsi; ?></textarea>
            </div>
            <div class="mb-8">
                <label class="block text-sm font-semibold text-text-main mb-2 flex items-center gap-2">
                ...
                <select name="status" class="w-full px-4 py-3 rounded-xl border border-border-subtle focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary bg-surface shadow-sm transition">
                    <option value="aktif" <?php echo (isset($kategori->status) && $kategori->status == 'aktif') ? 'selected' : ''; ?>>Aktif</option>
                    <option value="nonaktif" <?php echo (isset($kategori->status) && $kategori->status == 'nonaktif') ? 'selected' : ''; ?>>Nonaktif</option>
                </select>
            </div>
